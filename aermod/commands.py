...
def run_task(task_id):
    ...
    Task must be run in the same server used to run selected wrf task.
    '''
    # read the task
    try:
    ...
    target_server_client = rpc_client(task.setting.wrf_task.queue.server_id)
    ...
    if target_server is not None:
        ...
        target_server_client.aermod.add_job(data)
...
def rerun_task(task_id):
    ...
    if target_server is not None:
        ...
        if jobqueue.envid is not None:
            target_server_client.aermod.cleanupenv(jobqueue.envid)
        target_server_client.aermod.add_job(data, jobqueue.envid)
...
def stop_task(task_id):
    ...
    try:
        if target_server_client.aermod.stop_job(taskqueue.envid):
