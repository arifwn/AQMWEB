...
stream_gallery.reset = function(gallery_id){
    // Reset the gallery to initial state. Useful when changing content dynamically
    try {
        clearInterval(stream_gallery[gallery_id].play_timer);
    }
    catch(e){
        
    }
    
    stream_gallery[gallery_id].is_playing = false;
    $('#' + gallery_id + ' .playcontrol .play').show();
    $('#' + gallery_id + ' .playcontrol .pause').hide();
    
    stream_gallery[gallery_id].maxnum = $('#'+gallery_id+' .viewport .viewlist li').length;
    stream_gallery[gallery_id].num = 1;
    stream_gallery[gallery_id].play_timer = null;
    stream_gallery[gallery_id].is_playing = false;
    
    // setup nav list
    stream_gallery.create_nav_list(gallery_id);
    
    // setup first view
    stream_gallery.goto_target(gallery_id, 1);
};

stream_gallery.create_nav_list = function(gallery_id){
    // create navigation list
    $('#' + gallery_id + ' .navlist').empty();
    items = $('#' + gallery_id + ' .viewlist li');
    for(var i=0; i<items.length; i++){
        var index = i + 1;
        var description = $(items[i]).attr('data-item-description');
        var html = '<li><a href="#' + gallery_id + '" data-stream-gallery-num="' + index + '">' + description + '</a></li>';
        if(i == 0) {
            html = '<li><a href="#' + gallery_id + '" class="active" data-stream-gallery-num="' + index + '">' + description + '</a></li>';
        }
        $('#' + gallery_id + ' .navlist').append(html);
    }
    
    // add event handler
    $('#'+gallery_id+' .navigation .navlist li a').click(function(e){
        e.preventDefault();
        var target_num = parseInt($(this).attr('data-stream-gallery-num'));
        // display targetted image
        stream_gallery.goto_target(gallery_id, target_num);
    });
};
...
(function( $ ){
    ...
    $.fn.stream_gallery = function ( selector ) {
        return this.each(function() {
            ...
            //list navigation
            stream_gallery.create_nav_list(id_name);
            ...
            stream_gallery.goto_target(id_name, 1);
